...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
  ...
  var grundschulenGeoJSON = getGeoresourceByName('Grundschulen', georesourcesMap);

  log("Retrieved required georesources successfully");

  var numFeatures = targetSpatialUnit_geoJSON.features.length;

  // now we compute the new indicator
  log("replace feature geometries of targetSpatialUnit with their center point of mass.");

  // now we compute the new indicator
  log("Compute indicator for a total amount of " + numFeatures + " features by calculating the nearest school using footway paths.");
