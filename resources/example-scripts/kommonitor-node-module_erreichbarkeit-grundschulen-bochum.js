...
// CUSTOM CONSTANTS
const schoolTypeAttrName = "COLL_NAME";
const schoolTypeAttrTargetValue = "Grundschulen";
...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
  ...
  var wohngeb = getGeoresourceByName("Wohngebäude", georesourcesMap);
  var schulen = getGeoresourceByName("Bildungseinrichtungen", georesourcesMap);

  // divide by 1000 for meters-->kilometers
  var maxDistance = getProcessParameterByName_asNumber("MaxDistance", processParameters);
  console.log("max distance parameter in m: " + maxDistance);
  ...
  console.log("Filter for only preliminary schools");

  schulen.features.forEach(function(feature) {

    if(feature.properties[schoolTypeAttrName] === schoolTypeAttrTargetValue){
    	grundschulen.push(feature);
    }

  });

console.log("create distance isochrones for preliminary schools");

// isochrones by distance of 1000 m using foot-walking as GeoJSON feature collection
var isochrones_grundschulen = await isochrones_byDistance(grundschulen, "PEDESTRIAN", maxDistance);

console.log("Compute area for each building as proxy for wohnfläche");
wohngeb = area_featureCollection_asProperty(wohngeb);

console.log("get centroids of buildings");
var wohngeb_centroids = new Array();
wohngeb.features.forEach(function(feature){
  wohngeb_centroids.push(center_mass(feature, feature.properties));
});
...
var logProgressIndexSeparator = Math.round(wohngebLength / 100 * 10);
    ...
    for (var featureIndex=0; featureIndex < targetSpatialUnit_geoJSON.features.length; featureIndex++){
      ...
      if (within(wohngebFeature, spatialUnitFeat)){
        ...
        // pointIndex--;
  			...
  			for (var isochroneIndex = 0; isochroneIndex < isochrones_grundschulen.features.length; isochroneIndex++){
  				...
  				if(within(wohngebFeature, isochrone_grundschule_feature)){
    ...
    if(pointIndex % logProgressIndexSeparator === 0){
        console.log("PROGRESS: Compared '" + pointIndex + "' of total '" + wohngebLength + "' buildings to school isochrones.");
    }
	}

  targetSpatialUnit_geoJSON.features.forEach(function(spatialUnitFeature) {
    var indicatorValue = spatialUnitFeature.properties.wohnflCovered / spatialUnitFeature.properties.wohnflTotal || 0;
    spatialUnitFeature = setIndicatorValue(spatialUnitFeature, targetDate, indicatorValue);
