...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
  ...
  var autobahnauffahrten = KmHelper.getGeoresourceById("072b9f63-e0b7-4557-a3c5-d1fc34e703f5", georesourcesMap);

  // divide by 1000 for meters-->kilometers
  var travelTimeInS = KmHelper.getProcessParameterByName_asNumber("travelTimeInS", processParameters);
  KmHelper.log("travel time in seconds parameter: " + travelTimeInS);

  KmHelper.log("Filter for only autobahnauffahrten");

KmHelper.log("create distance isochrones for autobahnauffahrten");

// isochrones by distance of 1000 m using foot-walking as GeoJSON feature collection
var isochrones_autobahnauffahrten = await KmHelper.isochrones_byTime(autobahnauffahrten.features, "CAR", travelTimeInS, null, true, false, "highways");
...
var logProgressIndexSeparator = Math.round(wohngebLength / 100 * 10);
    ...
    for (var featureIndex=0; featureIndex < targetSpatialUnit_geoJSON.features.length; featureIndex++){
      ...
      if (KmHelper.within(wohngebFeature, spatialUnitFeat)){
        ...
        // pointIndex--;
  			...
  			// for each isochrones_autobahnauffahrten feature check if wohngebFeature lies within it
  			for (var isochroneIndex = 0; isochroneIndex < isochrones_autobahnauffahrten.features.length; isochroneIndex++){

  				var isochrone_autobahnauffahrt_feature = isochrones_autobahnauffahrten.features[isochroneIndex];

  				if(KmHelper.within(wohngebFeature, isochrone_autobahnauffahrt_feature)){
    ...
    if(pointIndex % logProgressIndexSeparator === 0){
        KmHelper.log("PROGRESS: Compared '" + pointIndex + "' of total '" + wohngebLength + "' buildings to autobahnauffahrt isochrones.");
    }
	}
