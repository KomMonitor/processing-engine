...
function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
  ...
  log("Iterate over base indicators and save intermediate values within a map object");
  ...
  */
  ...
  log("Process base indicator 'Gesamteinwohnerzahl'");
  ...
  */
  ewzGeoJSON.features.forEach(function(feature) {
    ...
    var featureId = getSpatialUnitFeatureIdValue(feature);
    // get the time series value of the base indicator feature for the requested target date (with its required prefix!)
    var einwohnerzahl = getIndicatorValue(feature, targetDate);

    if(einwohnerzahl === undefined || einwohnerzahl === null){
      log("WARNING: the feature with featureID '" + featureId + "' does not contain a time series value for targetDate '" + targetDate + "'");
      log("WARNING: the feature value will thus be set to '0' and computation will continue");
      einwohnerzahl = 0;
    }
  ...
  // now we compute the new indicator
  log("Compute indicator for a total amount of " + numFeatures + " features");
  ...
  targetSpatialUnit_geoJSON.features.forEach(function(spatialUnitFeature) {
    ...
    spatialUnitFeature = setIndicatorValue(spatialUnitFeature, targetDate, dichte);
