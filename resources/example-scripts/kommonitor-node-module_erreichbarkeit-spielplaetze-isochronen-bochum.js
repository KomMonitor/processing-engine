...
// CUSTOM CONSTANTS
const spielplatzTypeAttrName = "COLL_NAME";
const spielplatzTypeAttrTargetValueArray = ["Bolzpl", "Spielpl", "spielpl"];
...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
  ...
  var angebote = KmHelper.getGeoresourceById("f4ff30a7-9cea-4201-bb91-3a9fc9247e7b", georesourcesMap);
  ...
  KmHelper.log("Filter for only spielplaetze");

  angebote.features.forEach(function(feature) {

    // one line solution to check if the features property value contains any of the substring from the array
    // to get all spielplatz relevant angebote
    if(spielplatzTypeAttrTargetValueArray.some(substring => feature.properties[spielplatzTypeAttrName].includes(substring))){
    	spielplaetze.push(feature);
    }
...
var isochrones_spielplaetze = await KmHelper.isochrones_byDistance(spielplaetze, "PEDESTRIAN", maxDistance, true);
