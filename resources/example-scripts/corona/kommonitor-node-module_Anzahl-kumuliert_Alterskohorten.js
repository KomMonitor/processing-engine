...
const parameterName_georesourceId = "GEORESOURCE_ID";
const parameterName_startDate = "GEORESOURCE_START_DATE";
const parameterName_ageRange = "AGE_RANGE";
const parameterName_agePropertyName = "AGE_PROPERTY_NAME";
...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
  ...
  var covidStatusPointsId = KmHelper.getProcessParameterByName_asString(parameterName_georesourceId, processParameters);
  var covidStatusPointsStartDatePropertyName = KmHelper.getProcessParameterByName_asString(parameterName_startDate, processParameters);

  var agePropertyName = KmHelper.getProcessParameterByName_asString(parameterName_agePropertyName, processParameters);
  var ageRange = KmHelper.getProcessParameterByName_asString(parameterName_ageRange, processParameters);

  var age_min;
  var age_max;

  if(ageRange.includes("-")){
    age_min = Number(ageRange.split("-")[0]);
    age_max = Number(ageRange.split("-")[1]);
  }
  ...
  // var covid_status_points = KmHelper.getGeoresourceById(covidStatusPointsId, georesourcesMap);
...
var covid_status_points = await axios.get(kommonitorDataManagementURL + "/georesources/" + covidStatusPointsId + "/allFeatures")
  ...
  // covid_status_points = JSON.parse(JSON.stringify(covid_status_points));

  var totalNumber = covid_status_points.features.length;

    KmHelper.log("Filtering features by age '" + age_min + "' - '" + age_max + "'.");
  
    covid_status_points.features = covid_status_points.features.filter(feature => {
      var age = Number.parseFloat(KmHelper.getPropertyValue(feature, agePropertyName));
    ...
    var filteredNumber = covid_status_points.features.length;
  
    KmHelper.log("Detected '" + filteredNumber + "' features of total '" + totalNumber + "' features within age range '" + age_min + "' - '" + age_max + "'.");

  KmHelper.log("Filtering features by startDate.");

  var targetDate_asDate = new Date(targetDate);

  covid_status_points.features = covid_status_points.features.filter(feature => {
    var startDate = KmHelper.getPropertyValue(feature, covidStatusPointsStartDatePropertyName);
    if (startDate && startDate != null && (new Date(startDate) <= targetDate_asDate)){
      return true;
    }
    return false;
  });

  filteredNumber = covid_status_points.features.length;

  KmHelper.log("Detected '" + filteredNumber + "' features of total '" + totalNumber + "' features.");
...
var logProgressIndexSeparator = Math.round(targetSpatialUnit_geoJSON.features.length / 100 * 10);
    ...
    for (var featureIndex=0; featureIndex < targetSpatialUnit_geoJSON.features.length; featureIndex++){
      ...
      var pointsWithinFeature = KmHelper.pointsWithinPolygon(covid_status_points, spatialUnitFeat);
