...
const targetDate_habitants = "2019-12-31";

const parameterName_habitantsTargetDate = "HABITANTS_TARGET_DATE";

const parameterName_habitantsIndicatorId = "HABITANTS_ID";
const parameterName_covidIndicatorId = "COVID_ID";
...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
      ...
      var covidStatusIndicatorId = KmHelper.getProcessParameterByName_asString(parameterName_covidIndicatorId, processParameters);
      var habitantsIndicatorId = KmHelper.getProcessParameterByName_asString(parameterName_habitantsIndicatorId, processParameters);
      var habitantsTargetDate = KmHelper.getProcessParameterByName_asString(parameterName_habitantsTargetDate, processParameters);
  
      // retrieve required baseIndicator
      var habitantsGeoJSON = KmHelper.getBaseIndicatorById(habitantsIndicatorId, baseIndicatorsMap);
      var covidGeoJSON = KmHelper.getBaseIndicatorById(covidStatusIndicatorId, baseIndicatorsMap);
      ...
      */
      ...
      KmHelper.log("Process base indicator 'habitants'");
      ...
      */
      habitantsGeoJSON.features.forEach(function(feature) {
        ...
        var einwohnerzahl = KmHelper.getIndicatorValue(feature, habitantsTargetDate);

        if(einwohnerzahl === undefined || einwohnerzahl === null){
          KmHelper.log("WARNING: the feature with featureID '" + featureId + "' does not contain a time series value for targetDate '" + habitantsTargetDate + "'");
      ...
      KmHelper.log("Process base indicator 'covid'");

      /**
      * iterate over each feature of the baseIndicator and use its indicator value to modify map object
      * NOTE use spatialUnitFeatureId as key to be able to identify entries by their unique feature id!
      */
      covidGeoJSON.features.forEach(function(feature) {
        // get the unique featureID of the spatial unit feature as String
        var featureId = KmHelper.getSpatialUnitFeatureIdValue(feature);
        // get the time series value of the base indicator feature for the requested target date (with its required prefix!)
        var anzahlwahlberechtigte = KmHelper.getIndicatorValue(feature, targetDate);

        if(anzahlwahlberechtigte === undefined || anzahlwahlberechtigte === null){
        ...
        var anteilwahlberechtigte_percent = (anzahlwahlberechtigte / ewz) * 100;
        mapEntry.indicatorValue = anteilwahlberechtigte_percent;
