...
const parameterName_baseIndicatorId = "BASE_INDICATOR_ID";
...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
      ...
      var baseIndicatorId = KmHelper.getProcessParameterByName_asString(parameterName_baseIndicatorId, processParameters);

      // retrieve required baseIndicator
      var baseIndicatorGeoJSON = KmHelper.getBaseIndicatorById(baseIndicatorId, baseIndicatorsMap);
      ...
      */
      ...
      KmHelper.log("Process base indicator");
      ...
      */
      baseIndicatorGeoJSON.features.forEach(function(feature) {
        ...
        var indicatorValue_targetDate = KmHelper.getIndicatorValue(feature, targetDate);

        var targetDate_asDate = new Date(targetDate);
        targetDate_asDate.setDate( targetDate_asDate.getDate() - 1 );
        var targetDate_minus1Day_asDate = targetDate_asDate;
        var year = targetDate_minus1Day_asDate.getFullYear();
        var month = targetDate_minus1Day_asDate.getMonth() + 1;
        if(month < 10){
          month = "0" + month;
        }
        var day = targetDate_minus1Day_asDate.getDate();
        if(day < 10){
          day = "0" + day;
        }
        var targetDate_minus1Day_asString = "" + year + "-" + month + "-" + day;

        var indicatorValue_targetDate_minus1Day = KmHelper.getIndicatorValue(feature, targetDate_minus1Day_asString);

        if(indicatorValue_targetDate_minus1Day == undefined || indicatorValue_targetDate_minus1Day == null){
          KmHelper.log("WARNING: the feature with featureID '" + featureId + "' does not contain a time series value for targetDate minus 1 day'");
          KmHelper.log("WARNING: process will return");
          return;
        }

        if(indicatorValue_targetDate === undefined || indicatorValue_targetDate === null){
        ...
        var indicatorValue = Number(indicatorValue_targetDate) - Number(indicatorValue_targetDate_minus1Day);
