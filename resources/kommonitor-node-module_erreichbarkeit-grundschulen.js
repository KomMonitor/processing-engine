...
const schoolTypeAttrName = "S_SCHULFOR";
const schoolTypeAttrTargetValue = "Grundschule";
...
function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processProperties){
  ...
  var schulen = georesourcesMap.get('Schulen');
  ...
  grundschulen = turf.featureCollection(grundschulen);


console.log("create buffers");

// create buffers around freifl
var buffer_grundschulen = turf.buffer(grundschulen, maxDistance);

var buffer_grundschulen_featuresArray = buffer_grundschulen.features;
console.log(buffer_grundschulen_featuresArray.length);

var buffers_grundschulen_featureCollection = turf.featureCollection(buffer_grundschulen_featuresArray);
    ...
    for (var featureIndex=0; featureIndex < targetSpatialUnit_geoJSON.features.length; featureIndex++){
      ...
      if (turf.booleanWithin(wohngebFeature, spatialUnitFeat)){
        ...
        // pointIndex--;
  			...
  			for (var bufferIndex = 0; bufferIndex < buffers_grundschulen_featureCollection.features.length; bufferIndex++){

  				var buffer_grundschule_feature = buffers_grundschulen_featureCollection.features[bufferIndex];

  				if(turf.booleanWithin(wohngebFeature, buffer_grundschule_feature)){
