...
function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processProperties){
  ...
  var spielplaetze = georesourcesMap.get('Spielplätze');
...
var buffer_spielplaetze = turf.buffer(spielplaetze, maxDistance);

var buffer_spielplaetze_featuresArray = buffer_spielplaetze.features;
console.log(buffer_spielplaetze_featuresArray.length);

var buffers_spielplaetze_featureCollection = turf.featureCollection(buffer_spielplaetze_featuresArray);
    ...
    for (var featureIndex=0; featureIndex < targetSpatialUnit_geoJSON.features.length; featureIndex++){
      ...
      if (turf.booleanWithin(wohngebFeature, spatialUnitFeat)){
        ...
        // pointIndex--;
  			...
  			for (var bufferIndex = 0; bufferIndex < buffers_spielplaetze_featureCollection.features.length; bufferIndex++){

  				var buffer_spielplatz_feature = buffers_spielplaetze_featureCollection.features[bufferIndex];

  				if(turf.booleanWithin(wohngebFeature, buffer_spielplatz_feature)){
