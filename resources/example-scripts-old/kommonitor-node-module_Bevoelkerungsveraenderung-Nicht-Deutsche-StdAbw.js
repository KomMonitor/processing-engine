...
function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processProperties){
  ...
  var ewzGeoJSON = baseIndicatorsMap.get('Einwohnerzahl mit ausschließlich nicht-deutscher Staatsbürgerschaft - Prozentuale Veränderung');
  ...
  var year_tenBefore = specifiedYear - 10;
  var year_fifteenBefore = specifiedYear - 15;
  var date_fiveBefore = year_fiveBefore + '-' + targetDateArray[1] + '-' + targetDateArray[2];
  var date_tenBefore = year_tenBefore + '-' + targetDateArray[1] + '-' + targetDateArray[2];

  console.log('Target Date: ' + targetDate);
  console.log('Required Date before 5 years: ' + date_fiveBefore);
  console.log('Required Date before 10 years: ' + date_tenBefore);
  targetDate = indicator_date_prefix + targetDate;
  date_fiveBefore = indicator_date_prefix + date_fiveBefore;
  date_tenBefore = indicator_date_prefix + date_tenBefore;
  ...
  targetSpatialUnit_geoJSON.features.forEach(function(targetSpatialUnitFeature){
    ewzGeoJSON.features.forEach(function(ewzFeature){
      ...
      if (String(targetSpatialUnitFeature.properties[spatialUnitFeatureIdPropertyName]) === String(ewzFeature.properties[spatialUnitFeatureIdPropertyName])){
        ...
        var valueTargetDate = ewzFeature.properties[targetDate];
        var valueDateBeforeFive = ewzFeature.properties[date_fiveBefore];
        var valueDateBeforeTen = ewzFeature.properties[date_tenBefore];

        targetSpatialUnitFeature.properties[targetDate] = jStat.stdev([valueTargetDate, valueDateBeforeFive, valueDateBeforeTen]);
      }
    });
  });
